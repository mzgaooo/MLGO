from layer import SGP, linear, graph_constructor, TimeConv
import torchdiffeq
import torch
...
class MPBlock(nn.Module):
    ...
    def __init__(self, hidden_channels, num_hidden_layers, device, dropout, method_2, time_2,
        ...
        self.gconv_1 = SGP(c_hidden_channels=hidden_channels, init_alpha=alpha, method=method_2, time=time_2, 
                           step_size=step_size_2, device=device, rtol=rtol, atol=atol, dropout=dropout)
        self.gconv_2 = SGP(c_hidden_channels=hidden_channels, init_alpha=alpha, method=method_2, time=time_2, 
                           step_size=step_size_2, device=device, rtol=rtol, atol=atol, dropout=dropout)
        self.gconv_3 = SGP(c_hidden_channels=hidden_channels, init_alpha=alpha, method=method_2, time=time_2, 
                           step_size=step_size_2, device=device, rtol=rtol, atol=atol, dropout=dropout)
        self.gconv_4 = SGP(c_hidden_channels=hidden_channels, init_alpha=alpha, method=method_2, time=time_2, 
                           step_size=step_size_2, device=device, rtol=rtol, atol=atol, dropout=dropout)
    ...
    def forward(self, t, x):
        ...
        x0 = F.relu(self.mlp1(torch.cat((x[2], x[0]), dim=1)))
...
class MLGO(nn.Module):
    def __init__(self, num_nodes, device, predefined_A=None, static_feat=None, dropout=0.3, num_hidden_layers=3,
        ...
        self.affine_weight = nn.Parameter(torch.Tensor(*(conv_channels, num_nodes)))  # C*H
        self.affine_bias = nn.Parameter(torch.Tensor(*(conv_channels, num_nodes)))  # C*H

        self.ODE = ODEBlock(MPBlock(hidden_channels=conv_channels, num_hidden_layers=num_hidden_layers, device=device,
                                    dropout=dropout, method_2=method_2, time_2=time_2, step_size_2=step_size_2,
    ...
    def forward(self, input, idx=None):
        seq_len = input.size(3)  #input：B*dim1*n*窗口
        assert seq_len == self.seq_length, 'input sequence length not equal to preset sequence length'

        x = self.start_conv(input) #B*dim64*n*窗口

        adp1 = torch.from_numpy(self.predefined_A[0].astype(np.float32))  #dtw
        adp2 = torch.from_numpy(self.predefined_A[1].astype(np.float32))  #位置
        if idx is None:
